...
export function CreatorsPage() {
  ...
  return <section className={styles.page}>
    ...
    {creators.isPending ? <div className={styles.state}>Загружаем список…</div> : creators.isError ? <div className={styles.error}>Не удалось получить креаторов: {creators.error.message}</div> : creators.data.items.length === 0 ? <div className={styles.empty}><h2>Креаторов пока нет</h2><p>Создайте первую карточку, затем добавьте контакты и подключите аккаунты платформ.</p><Button onClick={() => setOpen(true)}>Добавить первого креатора</Button></div> : visibleCreators.length === 0 ? <div className={styles.state}>В этой компании креаторов пока нет.</div> : <div className={styles.table}><div className={styles.tableHead}><span>Креатор</span><span>Telegram</span><span>Компания</span><span>Статус</span><span>Работы</span></div>{visibleCreators.map((creator) => <Link className={styles.creatorRow} key={creator.id} to={`/app/creators/${creator.id}`}><div className={styles.creator}><span className={styles.avatar}>{creator.displayName.slice(0, 1)}</span><div><b>{creator.displayName}</b><small>{[creator.lastName, creator.firstName, creator.middleName].filter(Boolean).join(' ')}</small></div></div><span className={creator.telegramUsername ? styles.telegram : styles.unassigned}>{creator.telegramUsername ? `@${creator.telegramUsername}` : 'Не указан'}</span><span className={creator.companyName ? styles.company : styles.unassigned}>{creator.companyName || 'Без компании'}</span><span className={`${statusStyles.status} ${creator.status === 'ACTIVE' ? statusStyles.active : creator.status === 'ON_LEAVE' ? statusStyles.onLeave : statusStyles.dismissed}`}>{statusLabels[creator.status]}</span><span className={creator.workStatus === 'OK' ? styles.workOk : creator.workStatus === 'IN_PROGRESS' ? styles.workInProgress : styles.workAttention}>{creator.workStatus === 'OK' ? 'Всё ок' : creator.workStatus === 'IN_PROGRESS' ? `В работе · ${creator.workComment}` : creator.workComment}</span></Link>)}</div>}
