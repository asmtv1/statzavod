...
export function CreatorsPage() {
  ...
  const { scopeKey, hasCapability, role, principal } = useAccess()
  ...
  const creators = useQuery({ queryKey: ['creators', scopeKey, scope, locale], queryFn: scope === 'archived' ? api.archivedCreators : api.creators })
  const companies = useQuery({ queryKey: ['companies', scopeKey, locale], queryFn: api.companies })
  const activeCompanyID = principal?.activeContext.companyId ?? ''
  const activeCompanyName = principal?.companies.find(company => company.id === activeCompanyID)?.name ?? ''
  const canCreate = hasCapability('CREATOR_CREATE') && Boolean(activeCompanyID)
  const visibleCreators = (creators.data?.items.filter(creator => companyFilter === 'ALL' || creator.companyId === companyFilter) ?? []).sort((left, right) => {
  ...
  function openCreate(){ if(!canCreate)return; setForm({...emptyForm,companyId:activeCompanyID}); setOpen(true) }
  function closeCreate(){ if(!create.isPending)setOpen(false) }
  function submit(event: FormEvent) { event.preventDefault(); setError(''); if(form.companyId)create.mutate(form) }
  useDialogFocus(open && role !== 'CREATOR' && canCreate, createDialogRef, closeCreate)

  return <section className={styles.page}>
    <header className={styles.header}><div><p className={styles.eyebrow}>{t('БАЗА КРЕАТОРОВ')}</p><h1>{t('Креаторы')}</h1><p>{t('Управляйте карточками креаторов и подключёнными аккаунтами.')}</p></div>{scope === 'active' && hasCapability('CREATOR_CREATE') ? <Button onClick={openCreate} disabled={!canCreate} title={!activeCompanyID?t('Сначала выберите компанию в рабочем контексте'):undefined}>{t('Добавить креатора')}</Button> : null}</header>
    {scope==='active'&&hasCapability('CREATOR_CREATE')&&!activeCompanyID?<div className={styles.notice}>{t('Чтобы создать креатора, выберите активную компанию в меню слева.')}</div>:null}
    <div className={styles.filters}>
      <div className={styles.scopeTabs} aria-label={t('Раздел креаторов')}><button type="button" className={scope === 'active' ? styles.scopeActive : ''} onClick={() => { setScope('active'); setCompanyFilter('ALL') }}>{t('Рабочие')}</button>{hasCapability('CREATOR_ARCHIVE') ? <button type="button" className={scope === 'archived' ? styles.scopeActive : ''} onClick={() => { setScope('archived'); setCompanyFilter('ALL') }}>{t('Архив')}</button> : null}</div>
      {creators.data?.items.length ? <><label>{t('Компания')}<select value={companyFilter} onChange={event => setCompanyFilter(event.target.value)}><option value="ALL">{t('Все компании')}</option>{companies.data?.items.map(company => <option value={company.id} key={company.id}>{company.name}</option>)}</select></label><label>{t('Сортировка работ')}<select value={workSort} onChange={event => setWorkSort(event.target.value as WorkSort)}><option value="DEFAULT">{t('Без сортировки')}</option><option value="OK">{t('Сверху: всё ок')}</option><option value="NEEDS_ATTENTION">{t('Сверху: нужны работы')}</option><option value="IN_PROGRESS">{t('Сверху: в работе')}</option></select></label></> : null}
      <span>{t('Найдено:')} {visibleCreators.length}</span>
    </div>
    {creators.isPending ? <div className={styles.state}>{t('Загружаем список…')}</div> : creators.isError ? <div className={styles.error}>{t('Не удалось получить креаторов:')} {t(creators.error.message)}</div> : creators.data.items.length === 0 ? scope === 'archived' ? <div className={styles.empty}><h2>{t('Архив пуст')}</h2><p>{t('Сюда попадут карточки, убранные из рабочих списков. Их можно будет восстановить в любой момент.')}</p></div> : <div className={styles.empty}><h2>{t('Креаторов пока нет')}</h2><p>{t('Создайте первую карточку, затем добавьте контакты и подключите аккаунты платформ.')}</p>{hasCapability('CREATOR_CREATE')?<Button onClick={openCreate} disabled={!canCreate}>{t('Добавить первого креатора')}</Button>:null}</div> : visibleCreators.length === 0 ? <div className={styles.state}>{t('В этой компании креаторов пока нет.')}</div> : <div className={styles.table}>
      ...
      {visibleCreators.map(creator => <Link className={styles.creatorRow} key={creator.id} to={`/app/creators/${creator.id}`}><div className={styles.creator}><span className={styles.avatar}>{creator.displayName.slice(0, 1)}</span><div><b>{creator.displayName}</b><small>{creator.archivedAt ? `${t('В архиве с')} ${archiveDateFormatter.format(new Date(creator.archivedAt))}` : [creator.lastName, creator.firstName, creator.middleName].filter(Boolean).join(' ')}</small></div></div><span className={creator.telegramUsername ? styles.telegram : styles.unassigned}>{creator.telegramUsername ? `@${creator.telegramUsername}` : t('Не указан')}</span><span className={creator.companyName ? styles.company : styles.unassigned}>{creator.companyName || t('Компания недоступна')}</span><CreatorPlatforms platforms={creator.connectedPlatforms}/><span className={`${statusStyles.status} ${creator.status === 'ACTIVE' ? statusStyles.active : creator.status === 'ON_LEAVE' ? statusStyles.onLeave : statusStyles.dismissed}`}>{statusLabel(creator.status)}</span><CreatorWorkCell workStatus={creator.workStatus} workComment={creator.workComment}/></Link>)}
    </div>}
    {open && role !== 'CREATOR' && canCreate ? <div className={styles.backdrop} role="presentation" onMouseDown={closeCreate}><form ref={createDialogRef} className={styles.dialog} role="dialog" aria-modal="true" aria-labelledby={createTitleID} tabIndex={-1} onSubmit={submit} onMouseDown={event => event.stopPropagation()}><div className={styles.dialogHead}><div><p className={styles.eyebrow}>{t('НОВАЯ КАРТОЧКА')}</p><h2 id={createTitleID}>{t('Добавить креатора')}</h2></div><button type="button" className={styles.close} aria-label={t('Закрыть')} onClick={closeCreate}>×</button></div><div className={styles.fields}><label>{t('Имя')}<input required value={form.firstName} onChange={e => setForm({ ...form, firstName: e.target.value })} /></label><label>{t('Фамилия')}<input required value={form.lastName} onChange={e => setForm({ ...form, lastName: e.target.value })} /></label><label>{t('Отчество')}<input value={form.middleName} onChange={e => setForm({ ...form, middleName: e.target.value })} /></label><label>{t('Отображаемое имя')}<input placeholder={t('Если не указать — сформируется автоматически')} value={form.displayName} onChange={e => setForm({ ...form, displayName: e.target.value })} /></label><label className={styles.note}>{t('Компания')}<input value={activeCompanyName} readOnly/><input type="hidden" name="companyId" value={form.companyId}/></label><label className={styles.note}>Telegram<input placeholder="@username or t.me/username" value={form.telegramUsername} onChange={e => setForm({ ...form, telegramUsername: e.target.value })} /></label><label className={styles.note}>{t('Внутренний комментарий')}<textarea value={form.internalNote} onChange={e => setForm({ ...form, internalNote: e.target.value })} rows={3} /></label></div>{error && <p className={styles.error}>{t(error)}</p>}<footer><button type="button" className={styles.cancel} onClick={closeCreate}>{t('Отмена')}</button><Button type="submit" disabled={create.isPending||!form.companyId}>{create.isPending ? t('Создаём…') : t('Создать креатора')}</Button></footer></form></div> : null}
  </section>
}
