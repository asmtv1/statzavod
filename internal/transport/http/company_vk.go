...
func (s *Server) listCompanyVKAccounts(w http.ResponseWriter, r *http.Request) {
	...
	for rows.Next() {
		var id, companyID, companyName, oauthDisplayName, oauthUsername, oauthAvatarURL, oauthProfileURL, oauthStatus, platformAccountID, syncError string
		...
		if err := rows.Scan(&id, &companyID, &companyName, &loginCiphertext, &loginNonce, &phoneCiphertext, &phoneNonce, &updatedAt, &oauthDisplayName, &oauthUsername, &oauthAvatarURL, &oauthProfileURL, &oauthStatus, &platformAccountID, &lastSyncedAt, &lastSuccessAt, &syncError, &consecutiveFailures, &communityCount); err != nil {
		...
		items = append(items, map[string]any{"id": id, "companyId": companyID, "companyName": companyName, "login": login, "phone": phone, "hasPassword": login != "", "accessMethod": accessMethod, "updatedAt": updatedAt, "oauthDisplayName": oauthDisplayName, "oauthUsername": oauthUsername, "oauthAvatarUrl": oauthAvatarURL, "oauthProfileUrl": oauthProfileURL, "oauthStatus": oauthStatus, "platformAccountId": platformAccountID, "lastSyncedAt": lastSyncedAt, "lastSuccessAt": lastSuccessAt, "syncError": syncError, "consecutiveFailures": consecutiveFailures, "communityCount": communityCount})
	}
