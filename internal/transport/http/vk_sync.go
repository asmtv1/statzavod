...
func (s *Server) upsertVKVideo(ctx context.Context, job platformSyncJob, creatorID string, video vkVideo) error {
	...
	err = tx.QueryRow(ctx, `INSERT INTO publications(creator_id,platform_account_id,platform,external_id,publication_type,title,description,permalink,thumbnail_url,published_at,status,metadata) VALUES($1,$2,'VK',$3,'VIDEO',$4,$5,$6,$7,$8,'ACTIVE',jsonb_build_object('player',$9::text)) ON CONFLICT(platform,external_id) DO UPDATE SET creator_id=excluded.creator_id,platform_account_id=excluded.platform_account_id,title=excluded.title,description=excluded.description,permalink=excluded.permalink,thumbnail_url=excluded.thumbnail_url,published_at=excluded.published_at,status='ACTIVE',metadata=excluded.metadata,updated_at=now() RETURNING id`, creatorID, job.AccountID, externalID, video.Title, video.Description, permalink, thumbnail, publishedAt, video.Player).Scan(&publicationID)
	if err != nil {
		return err
	}
